// Default zones if none are saved
const DEFAULT_ZONES = ['local', 'UTC', 'America/New_York'];
...
const timezoneListContainer = document.getElementById('timezoneList');
const statusDiv = document.getElementById('status');

// Load settings and populate list
...
function restoreOptions() {
  chrome.storage.sync.get({
    targetTimezones: DEFAULT_ZONES
  }, (items) => {
    renderCheckboxList(items.targetTimezones);
  });
}

function renderCheckboxList(savedZones) {
    if (!timezoneListContainer) return;
    timezoneListContainer.innerHTML = '';
    
    // Create 'local' option manually as it's not in IANA list
    createCheckboxItem('local', savedZones);

    // Create options for all IANA zones
    AVAILABLE_TIMEZONES.forEach(tz => {
        // Skip 'UTC' if it's already in the list (sometimes duplicated in source lists)
        if (tz === 'UTC' && timezoneListContainer.querySelector('input[value="UTC"]')) return;
        createCheckboxItem(tz, savedZones);
    });
}

function createCheckboxItem(value, savedZones) {
    const div = document.createElement('div');
    div.className = 'timezone-item';
    
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.id = `tz-${value}`;
    checkbox.value = value;
    checkbox.checked = savedZones.includes(value);
    ...
    label.htmlFor = `tz-${value}`;
    label.textContent = value;
    
    div.appendChild(checkbox);
    div.appendChild(label);
    timezoneListContainer.appendChild(div);
}

function updateSavedZones() {
    const checkboxes = timezoneListContainer.querySelectorAll('input[type="checkbox"]:checked');
    const selectedZones = Array.from(checkboxes).map(cb => cb.value);
    
    saveOptions(selectedZones);
}

function saveOptions(zones) {
  chrome.storage.sync.set({
    targetTimezones: zones
  }, () => {
